...
def logout():
    ...
    if session.get('user'):  # Used MS Login
        ...
        session.clear()
        # DONE: Also logout from your tenant's web session
...
def authorized():
    ...
    if request.args.get('code'):
        cache = _load_cache()
        # DONE: Acquire a token by authorization code from an MSAL app
...
def _build_msal_app(cache=None, authority=None):
    # DONE: Create and return a Confidential Client Application from msal
...
def _build_auth_url(authority=None, scopes=None, state=None):
    # DONE: Get the authorization request URL from a built msal app, and return it
